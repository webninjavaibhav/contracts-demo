...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const realResponse = await fetch(`${realBaseUrl}/compareVersions`, {
      method: "POST",
      body: data,
    });
    const parsedRealResponse = await realResponse.json();
    if (parsedRealResponse?.summary || parsedRealResponse?.comparisons) {
      resultantData = parsedRealResponse;
    } else {
      const mockResponse = await fetch(`${mockBaseUrl}/compareVersions`, {
        method: "POST",
        body: fileData,
      });
